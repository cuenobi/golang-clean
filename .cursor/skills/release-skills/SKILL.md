...
For `khaojai-ai`, apply these defaults unless the user explicitly overrides:
- Use root `VERSION` as release source of truth (Option C)
- Update `CHANGELOG.md`
- Sync `khaojai-ui/package.json` version to release version
- Do not create `.releaserc.yml`
- Create tag as `{VERSION}` (no `v` prefix)
...
**khaojai-ai override**:
- If workspace contains root `CHANGELOG.md` and `khaojai-ui/package.json`, prefer root `VERSION` as version file.
- If `VERSION` does not exist, create it during release.
- Do not add `.releaserc.yml` unless user explicitly requests it.
...
3. **khaojai-ai sync rule**:
   - Always sync `khaojai-ui/package.json` `$.version` to the same release version
...
For `khaojai-ai`: do not create `.releaserc.yml` by default. Use root `VERSION` + `CHANGELOG.md` + `khaojai-ui/package.json` sync instead.
